...
export type RemoteCompactionDetails = {
  version: 2;
  provider: "openai-codex-responses-compaction";
...
export function remoteCompactionV2EndpointUrl(model: Model<any>): string {
  if (!isOpenAICodexModel(model) && !isOpenAICodexLbModel(model)) {
    throw new Error("Remote compaction is only supported for OpenAI Codex models.");
  }

  const configuredBaseUrl = typeof model.baseUrl === "string" ? model.baseUrl : undefined;
  if (isOpenAICodexLbModel(model) && !configuredBaseUrl?.trim()) {
...
export function buildRemoteCompactionHeaders(params: {
  ...
  if (!isOpenAICodexModel(params.model) && !isOpenAICodexLbModel(params.model)) {
    throw new Error("Remote compaction headers are only supported for OpenAI Codex models.");
  }

  return {
    ...commonHeaders,
    ...(isOpenAICodexModel(params.model)
      ? { "chatgpt-account-id": extractCodexAccountId(params.apiKey) }
      : {}),
    originator: "pi",
    "user-agent": `pi-openai-codex-compaction (${platform()} ${release()}; ${arch()})`,
...
export async function generatePortableSummary(params: {
  ...
  const response = await params.completeModel(
    params.model,
    {
      messages: [
        {
...
export async function callRemoteCompactionEndpoint(params: {
  ...
  if (!isOpenAICodexModel(params.model) && !isOpenAICodexLbModel(params.model)) {
    throw new Error("Remote compaction is only enabled for OpenAI Codex models.");
  }
...
export function extractRemoteCompactionDetails(details: unknown):
  ...
  if (remote.provider !== "openai-codex-responses-compaction" || remote.version !== 2) {
    return undefined;
  }
...
export function buildCompactionSummaryText(model: Model<any>): string {
  return `OpenAI Codex remote compaction applied for ${model.provider}/${model.id}. Pi keeps this text summary for portability. Later compatible Codex turns use the provider-native replacement history stored in compaction details.`;
}
