...
export function normalizeBaseUrl(value: string): string {
	...
	if (url.pathname !== "/v1") {
		throw new Error(`${BASE_URL_ENV} (or ${LEGACY_BASE_URL_ENV}) must target codex-lb's /v1 API route`);
	}

	return url.toString().replace(/\/+$/, "");
}

/** Registers codex-lb through its API-key-compatible Responses endpoint. */
export default function openAICodexLbExtension(pi: ExtensionAPI) {
